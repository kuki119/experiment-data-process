...
#2017.4.27修改，增加筛面方程，较精确的选取筛面以上范围作为筛上物，筛分结束以斜率-1000为分界点
#以入料结束为起点，以仿真结束为重点，中间取10个点左右 即：(time1-time2)/10
import numpy as np
...
def main():
    #需要修改的地方！！
    ########################
    address = 'F:/self_orthogonal_experiment/data/test3'
    ...
    print(u'z矩阵的形状：{}'.format(np.shape(z_matrix)))
    ...
    dataPlot(x_matrix[num_time/2][1:],z_matrix[num_time/2][1:])
    ...
    [slope_1000,ite] = myfind(slope)
    end_time = time_list[ite]
    eff_end_time = eff_unit_time[ite]
    print(u'斜率大于-1000的第一个斜率为：{}'.format(slope_1000))
    print(u'时间点：{}'.format(end_time))
    print(u'该时刻的单位时间筛分效率：{}'.format(eff_end_time))

    #导出自定义的筛分结束时刻 和 该时刻的单位时间筛分效率
    address_export_txt = address+'/export.txt'
    f = open(address_export_txt,'w')
    f.write(str(end_time)+'\t'+str(eff_end_time))
    f.close()

    # #导出全部数据 导出时刻、筛分效率(eff1,eff2,eff)、未筛颗粒个数、所占百分比。
