...
#转换矩阵 [cos(a),sin(a);-sin(a),cos(a)]  二维……
    ...
    sinp = dp[1]/modp
    cosp = dp[0]/modp
    # con_matrix = np.array([[cosp,sinp],[-sinp,cosp]]) #二维转换矩阵
    # con_matrix = np.array([[cosp,0,-sinp],[0,1,0],[sinp,0,cosp]]).T #三维转换矩阵 绕y轴转p角度
    con_matrix = np.array([[cosp,0,-sinp,0],[0,1,0,0],[sinp,0,cosp,0],[0,0,0,1]]).T #四维转换矩阵  
...
############返回料层颗粒################
    ...
    return ptc_bed,z_min,z_label #返回料层颗粒 筛面z坐标 料层底面z坐标
