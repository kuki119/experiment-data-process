...
#计算单位时间筛分效率  传入一个时刻下的 颗粒数据 和 筛网数据
    ...
    mass_std = calStdMass(0.9) #传入指定直径返回响应的质量
...
#1、计算平均距离矩阵； 2、计算料层颗粒的平均直径； 3、计算松散系数
    ...
    mass = ptc_bed[:,3]
    # dns = 2678*10**(-6)  #密度2678千克每立方米 转成 克每立方毫米
    # c = np.pi/6
    # diams = (mass/(dns*c))**(1/3)
    diams = calDiam(mass)
    diams_ave = sum(diams)/num_bed
    por = dist_ave/diams_ave #松散系数 
    return por

def calStratification(ptc_bed,bed_z):
#############计算沉降差##############
#1、分别计算大小颗粒分层沉降系数  2、相减得到沉降差
    ...
    # mass_std = ((np.pi/6)*dns*diam**3)*1000 #把质量的kg单位变成g单位 分离粒径颗粒质量
    ...
    strat = dist_sml_ave/band #尝试用小颗粒平均位置与料层宽度之比表示

###############料层Z与料层中颗粒粒径 的正相关关系  斜率越大 分层效果越好#########
    # x = ptc_bed[:,2] #颗粒z轴坐标
