...
def dataParticle(path,x_name,z_name,mass_name):
#返回数据结构 以时刻分块的列表 一块是n维数组
    ...
    unit_list = [data_x.columns[1],data_z.columns[1],data_mass.columns[1]]
    print(unit_list)
    data_x.columns = ['a','x']
    data_z.columns = ['a','z']
    ...
    data_orig = concat([data_x,data_z.z,data_mass.mass],axis=1)
    ...
    for i in range(len(id_time)):
        try:
            data_times.append(data_orig.iloc[id_time[i]+2:id_time[i+1],:].as_matrix(columns=['x','z','mass']))
        except:
            data_times.append(data_orig.iloc[id_time[i]+2:,:].as_matrix(columns=['x','z','mass']))
